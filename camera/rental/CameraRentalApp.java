...
public class CameraRentalApp {
	...
	public void addCamera(Camera camera) { availableCameras.add(camera);
	}

	public void removeCamera(int cameraIndex) {
	if (cameraIndex >= 1 && cameraIndex <= availableCameras.size()) { availableCameras.remove(cameraIndex - 1); System.out.println("Camera removed successfully.");
	} else {
	System.out.println("Invalid camera selection.");
	}
	}

	public void displayAvailableCameras() { if (availableCameras.isEmpty()) {
	...
	for (int i = 0; i < availableCameras.size(); i++) { Camera camera = availableCameras.get(i);
	...
	selectedCamera.setRented(true); System.out.println("Camera rented successfully!");
	} else {
	...
	System.out.println("Deposit successful. Current wallet balance: $"
